...
{
    [SerializeField] Image gem1;
    [SerializeField] Image gem2;
    [SerializeField] Image gem3;
    [SerializeField] Image gem4;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.tag == "Chest1")
        {
            ...
            Destroy(collision.gameObject); // Destroy the chest after picking up gem1
        }
        else if (collision.gameObject.tag == "Chest2")
        {
            gem2.GetComponent<Image>().enabled = true;
            Destroy(collision.gameObject); // Destroy the chest after picking up gem2
        }
        else if (collision.gameObject.tag == "Chest3")
        {
            gem3.GetComponent<Image>().enabled = true;
            Destroy(collision.gameObject); // Destroy the chest after picking up gem3
        }
        else if (collision.gameObject.tag == "Chest4")
        {
            gem4.GetComponent<Image>().enabled = true;
            Destroy(collision.gameObject); // Destroy the chest after picking up gem4
        }
    }
